...
async function main(_config = {}) {
    ...
    console.log("-------------- Phase 1 ----------------")  
    await action(ohm, stakingHelper, users, [0], ['0.0001'])
    await userTable(ohm, sOHM, users)
    await log(0);
    ...
    const verySmall = ethers.utils.parseUnits('0.0001', 9)
    await sOHM.connect(user1).approve(staking.address, ethers.utils.parseUnits('1000000000000000000000', 9))
    for (let i = 0; i < 10; i++) {
        var sOhmBalance = await sOHM.balanceOf(user1.address);
        await staking.connect(user1).unstake(sOhmBalance - verySmall, false)
        await skipTime(50)
        await staking.rebase()
        await userTable(ohm, sOHM, users)
        await log(0);;
