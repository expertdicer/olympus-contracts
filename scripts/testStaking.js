...
async function main(_config = {}) {
    ...
    tx = await treasury.connect(governor).deposit(config.principalDeposit.amount, dai.address, config.principalDeposit.profit);
    console.log('Block rebase 1 : ', await ethers.provider.getBlockNumber())
    await staking.connect(governor).stake(ethers.utils.parseUnits('1000', 9), governor.address, true)
    const log = async (i) => {
        console.log('log:',
