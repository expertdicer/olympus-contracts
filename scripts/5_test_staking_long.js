...
async function main(_config = {}) {
    ...
    await skipTime(50);          
    // await log(0);
    // tx = await staking.connect(governor).rebase();
    // await log(0);
    ...
    var counter = 0;
    const total_rebase = 300;
    const random = true;
    var usersActions = [];
    if (!random) {
        // copy generated series of users' actions here
        usersActions = [[],];
    }

    var headers = ["Index", "Total supply", "Staked supply", "Next distribution"];
    var table = [headers];

    const unitLog = async function() {
        var log_decimals = 4;
        var log_index_decimals = 9 ;

        var epoch = await staking.epoch();

        var index = ethers.utils.formatUnits(await sOHM.index(), 9);
        var ts = ethers.utils.formatUnits(await ohm.totalSupply(), 9);
        var ss = ethers.utils.formatUnits(await sOHM.circulatingSupply(), 9);
        var distribute = ethers.utils.formatUnits(epoch.distribute, 9);

        var logs = [parseFloat(index).toFixed(log_index_decimals), parseFloat(ts).toFixed(log_decimals), parseFloat(ss).toFixed(log_decimals), parseFloat(distribute).toFixed(log_decimals)];

        for (let j = 0; j < users.length; j++) {
            var t_balance = ethers.utils.formatUnits(await ohm.balanceOf(users[j].address), 9);
            var xt_balance = ethers.utils.formatUnits(await sOHM.balanceOf(users[j].address), 9)
            logs.push(parseFloat(t_balance).toFixed(log_decimals)+ " - " + parseFloat(xt_balance).toFixed(log_decimals));
        }

        return logs;
    }
    table.push(await unitLog());
    console.log("--------- Start generating random behaviors -----------");
    
    var result = [];
    for (let i = 0; i < total_rebase; i++) {
        if(i % 10 == 0 && i / 10 > 0) {
            // var deposit = 50000;
            // var profit = 50000 * (i/10) * 0.05;
            var deposit = 100000;
            var profit = 50000;
            tx = await treasury.connect(governor).deposit(ethers.utils.parseUnits(String(deposit), 18), dai.address, ethers.utils.parseUnits(String(profit), 9));

            for (let i = 0; i < users.length; i++) {
                tx = await ohm.connect(governor).transfer(users[i].address, ethers.utils.parseUnits(String((deposit - profit)/users.length), 9));
                tx = await ohm.connect(users[i]).approve(staking.address, ethers.utils.parseUnits(String((deposit - profit)/users.length), 9));     
            }  
        } 

        var userActions = [];
        // Generate random users' action
        // 0: stake no claim
        // 1: stake and claim
        // 2: claim
        // 3: unstake
        ...
        if(random) {
            for (let j = 0; j < users.length; j++)
                userActions.push(await getRandomInt(users.length));
            usersActions.push(userActions);
        } else {
            userActions = usersActions[i];
        }
        ...
        for (let j = 0; j < users.length; j++) {
            ...
            switch(userActions[j]) {
                case 0:
                    tx = await ohm.connect(currentUser).approve(staking.address, ethers.utils.parseUnits(amount, 9));
                    tx = await staking.connect(currentUser).stake(ethers.utils.parseUnits(amount, 9), currentUser.address,  false);
                    break;
                case 1:
                    tx = await ohm.connect(currentUser).approve(stakingHelper.address, ethers.utils.parseUnits(amount, 9));
                    tx = await stakingHelper.connect(currentUser).stake(ethers.utils.parseUnits(amount, 9));
                    break;
                case 2:
                    tx = await staking.connect(currentUser).claim(currentUser.address);
                    break;
                case 3:
                    await sOHM.connect(currentUser).approve(staking.address, await sOHM.balanceOf(currentUser.address))
                    tx = await staking.connect(currentUser).unstake(await sOHM.balanceOf(currentUser.address), false);
                    break;
                case 4:
                    tx = await staking.connect(currentUser).forfeit();
                    break;
                default:
                    break;
            }
        ...
        if (true) {
            // Mine blocks and rebase
            await skipTime(50);

            tx = await staking.connect(governor).rebase();
            table.push(await unitLog());
        }
        result.push(userActions)
    }
